...
from lib.models.layers.multihead_attention import MultiheadAttention
from lib.models.layers.attention import MultiheadRelativeAttention
from .reversible_utils import *
...
class RevTransformerEncoderLayer(nn.Module):
    ...
    """
    def __init__(self, in_channels, hidden_dim, hidden_depth, attn_type, attn_head, attn_dropout, max_source_positions):
        super().__init__()

        self_attn = self.build_self_attention(hidden_dim, attn_type, attn_head, attn_dropout, max_source_positions)
        ...
        residual_fn_wrapper = RESIDUAL_FN_WRAPPERS['prenorm']
    ...
    def build_self_attention(self, embed_dim, attn_type, attn_head, attn_dropout, max_source_positions=1024):
        attn_cls = {
        ...
        return attn_cls(
            ...
            self_attention=True,
            max_position=max_source_positions
        )
    ...
    def reverse(self, x, encoder_padding_mask, attn_mask: Optional[Tensor] = None, rel_attn_kv: list = None):
        # every forward pass we sample a different seed
        # for dropout and save for forward fn in backward pass
        # to have correct dropout
        """ Y_1,            Y_2 """
...
class RevTransformerEnd(nn.Module):
    def __init__(self, in_channels, n_flows, hidden_dim, hidden_depth, attn_type, attn_head, attn_dropout, max_source_positions):
        super().__init__()
        self.n_flows = n_flows
        self.layers = nn.ModuleList(
            [RevTransformerEncoderLayer(in_channels, hidden_dim, hidden_depth, attn_type, attn_head, attn_dropout, max_source_positions)
             for _ in range(self.n_flows)]
        )

        self.shuffles = nn.ModuleList(
            [Shuffle(in_channels=in_channels * 2)
             for _ in range(self.n_flows)]
        )
...
class RevTransformerEncoder(nn.Module):
    def __init__(self, in_channels, n_flows, hidden_dim, hidden_depth, attn_type, attn_head, attn_dropout, max_source_positions, split_embedding=False, out_norm_type='actnorm'):
        super().__init__()

        self.domains = ['visual', 'textual']
        self.split_embedding = split_embedding
        self.out_norm_type = out_norm_type

        # build ends
        self.ends = nn.ModuleDict({
            domain: RevTransformerEnd(in_channels, n_flows, hidden_dim, hidden_depth, attn_type, attn_head, attn_dropout, max_source_positions) for domain in self.domains
        })

        feature_dim = in_channels * 2
        if self.out_norm_type == 'actnorm':
            _actnorm = ActNorm(feature_dim)
            self.actnorms = nn.ModuleDict({
                domain: _actnorm for domain in self.domains
            })

    def in_norm(self, x, domain):
        if not self.split_embedding:
            x = torch.cat([x, x], dim=-1)

        if self.out_norm_type == 'actnorm':
            return self.actnorms[domain](x)
        return x

    def out_norm(self, x, domain):
        o = x
        if self.out_norm_type == 'actnorm':
            o = self.actnorms[domain].reverse(o)
    ...
    def forward(self, x, src_domain='visual', target_domain='textual'):
        src_end = self.ends[src_domain]
        tgt_end = self.ends[target_domain]

        self.last_outs = []

        # 1) x_emb -> source end
        x = self.in_norm(x, domain=src_domain)

        # 2) encoding: run source end to get intermidiate representation
        mid, src_end_states = src_end.forward(x)
        self.last_outs.append(src_end_states)

        # 3) decoding: run target end to get output feature
        out, tgt_end_states = tgt_end.reverse(mid)
        self.last_outs.append(tgt_end_states)

        # 4) target end -> y_emb
        y = self.out_norm(out, domain=target_domain)
        self.last_outs.append(y)

        return y

    def reverse(self, out, src_domain='textual', target_domain='visual'):
        return self.forward(out, src_domain, target_domain)
