...
class LossComputation(nn.Module):
    ...
    def forward(
        ...
        if "cmpm_loss" in self.loss_type:
            loss.update({"cmpm_loss": self.loss_type["cmpm_loss"] * losses.cmpm_loss(visual_embed, textual_embed, labels)})

        if "cmpc_loss" in self.loss_type:
            loss.update({"cmpc_loss": self.loss_type["cmpc_loss"] * losses.cmpc_loss(self.projection, visual_embed, textual_embed, labels)})
