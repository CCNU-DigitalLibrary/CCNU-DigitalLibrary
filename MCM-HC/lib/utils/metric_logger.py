...
class MetricLogger(object):
    ...
    def __str__(self):
        ...
        for name, meter in self.meters.items():
            loss_str.append(
                "{}: {:.6f} ({:.6f})".format(name, meter.lastest, meter.global_avg)
            )
...
class TensorboardLogger(MetricLogger):
    ...
    def _get_tensorboard_writer(log_dir):
        try:
            ...
            raise ImportError(
                "To use tensorboard please install tensorboard "
        ...
        if self.writer:
            for k, v in kwargs.items():
                if isinstance(v, torch.Tensor):
                    v = v.item()
                assert isinstance(v, (float, int))
                self.writer.add_scalar(k, v, kwargs.get('iteration', self.iteration))
            self.iteration += 1
