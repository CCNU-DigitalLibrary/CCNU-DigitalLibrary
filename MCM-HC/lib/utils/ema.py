...
# exponential moving average from https://github.com/fadel/pytorch_ema/blob/master/torch_ema/ema.py
...
class ExponentialMovingAverage:
    ...
    Usage:
        ...
        >>> models = torch.nn.Linear(10, 2)
        >>> optimizer = torch.optim.Adam(models.parameters(), lr=1e-2)
        >>> ema = ExponentialMovingAverage(models.parameters(), decay=0.995)

        # Train for a few epochs
        >>> models.train()
        >>> for _ in range(20):
        >>>     logits = models(x_train)
        ...
        # (2) copies EMA parameters to models
    ...
    def __init__(
        ...
        # if the models goes out of scope but the ExponentialMovingAverage
        # is kept, no references to the models or its parameters will be
        # maintained, and the models will be cleaned up.
        ...
        if parameters is None:
            ...
            if any(p is None for p in parameters):
                raise ValueError(
                    ...
                    "the models to which they belong from being garbage "
                    "collected."
                )
            return parameters
        else:
