...
lis = os.listdir('data/first')

for ele in lis:
    imgData = []
    img = Image.open("data/first/" + ele)
    img = img.convert('RGB')
    ...
    if result[0].numpy() == 1:
        # "data/杂志V2_1001_2000/" + ele
        print("data/three/" + ele)
        temp_img = cv2.imread("data/first/" + ele)
        print(type(temp_img))
        if temp_img is None:
            print("图像为空")
            continue
            ...
            cv2.imwrite("deal_result/first/" + ele, temp_img)
