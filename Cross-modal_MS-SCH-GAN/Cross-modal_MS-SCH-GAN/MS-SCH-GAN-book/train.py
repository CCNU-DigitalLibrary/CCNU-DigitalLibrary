...
WORKDIR = '../book/'
GEN_MODEL_BEST_FILE = './model/gan_best_nn.model'
DIS_MODEL_BEST_FILE = './model/dis_best_nn.model'

GEN_MODEL_NEWEST_FILE = './model/gan_newest_nn.model'
DIS_MODEL_NEWEST_FILE = './model/dis_newest_nn.model'

GEN_MODEL_PRETRAIN_FILE = './model/gan_pretrain_nn.model'
DIS_MODEL_PRETRAIN_FILE = './model/dis_pretrain_nn.model'
...
feature_test_dict = ut.load_all_feature_for_train_test(WORKDIR + 'list/', WORKDIR + 'feature/')
...
def main():
	with tf.device('/gpu:' + str(GPU_ID)):
		...
		discriminator = DIS(IMAGE_DIM, TEXT_DIM, HIDDEN_DIM, OUTPUT_DIM, WEIGHT_DECAY, D_LEARNING_RATE, BETA, GAMMA, REWARD_BASE, param = dis_param)
		...
		for epoch in range(WHOLE_EPOCH):
			...
			for g_epoch in range(G_EPOCH):
				...
				if (g_epoch + 1) % (G_DISPLAY) == 0:
					i2t_test_map = MAP(sess, generator, test_i2t_pos, test_i2t, feature_test_dict, 'i2t')
					print('E%d G%d I2T_Test_MAP: %.4f' % (epoch, g_epoch, i2t_test_map))
					t2i_test_map = MAP(sess, generator, test_t2i_pos, test_t2i, feature_test_dict, 't2i')
			...
			for d_epoch in range(D_EPOCH):
				...
				if (d_epoch + 1) % (D_DISPLAY) == 0:
					i2t_test_map = MAP(sess, discriminator, test_i2t_pos, test_i2t, feature_test_dict, 'i2t')
					print('E%d D%d I2T_Test_MAP: %.4f' % (epoch, d_epoch, i2t_test_map))
					t2i_test_map = MAP(sess, discriminator, test_t2i_pos, test_t2i, feature_test_dict, 't2i')
